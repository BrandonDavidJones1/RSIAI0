...
class Seed_VMService:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None,
        ...
        self._core_real_probes = ['sh', 'pwd', 'ls', 'df', 'stat', 'grep', 'head', 'tail', 'top', 'cat', 'echo', 'rm', 'touch', 'mkdir', 'mv', 'cp', 'printf', 'python3', 'pip', 'pytest'] # Added pytest
...
"""
            ...
            if not posix_path.is_absolute():
                 ...
                 for part in combined_path.parts:
                     ...
                     if part == '..':
                         ...
                         elif not parts or parts == ['/']: continue
                     else:
                         parts.append(part)
    ...
    def _resolve_path(self, path_str: str) -> Optional[str]:
        ...
        try:
            cwd_path = PurePosixPath(current_dir); target_path: PurePosixPath
            if path_str == '~': target_path = PurePosixPath('/app')
            ...
            is_absolute_after_norm = resolved_path_str.startswith('/')
    ...
    def _sim_discover_tests(self, fs_state: Dict, base_paths: List[str], keyword_filter: Optional[str]) -> List[Tuple[str, str]]:
        ...
        discovered_tests = []
        # Default to /app/seed/tests if no paths given
        search_roots = base_paths if base_paths else [self._resolve_path("seed/tests") or "/app/seed/tests"]
        
        candidate_files = []
        for root_path_str in search_roots:
            resolved_root = self._resolve_path(root_path_str)
            if not resolved_root: continue

            if resolved_root in fs_state and fs_state[resolved_root].get('type') == 'directory':
                # It's a directory, find test_*.py files within it
                for path_key, item_info in fs_state.items():
                    if item_info.get('type') == 'file' and path_key.startswith(resolved_root + '/') and PurePosixPath(path_key).name.startswith("test_") and path_key.endswith(".py"):
                        candidate_files.append(path_key)
            elif resolved_root in fs_state and fs_state[resolved_root].get('type') == 'file' and PurePosixPath(resolved_root).name.startswith("test_") and resolved_root.endswith(".py"):
                # It's a specific test file
                candidate_files.append(resolved_root)

        candidate_files = list(set(candidate_files)) # Unique files
        ...
        for test_file_path in candidate_files:
            ...
            for line in content.splitlines():
                ...
                if match:
                    ...
                    if keyword_filter:
                        # Simple keyword check: in test name OR in file path
                        if keyword_filter in test_name or keyword_filter in test_file_path:
    ...
    def _sim_run_one_test(self, filepath: str, testname: str, keyword_filter: Optional[str]) -> Tuple[str, str]:
        ...
        if "expected_fail" in testname.lower():
            ...
            if "basic_core_tests" in keyword_filter and ("basic_core" in testname.lower() or "basic_core" in filepath.lower()):
                status = "PASSED" # basic_core_tests keyword implies pass for matching tests
            elif "internal_analysis" in keyword_filter and "internal_analysis" in testname.lower():
                 status = "PASSED"
            elif "internal_learning" in keyword_filter and "internal_learning" in testname.lower():
                 status = "PASSED"
            # Specific file path checks if keyword matches a suite from config
            elif "core" == keyword_filter and "test_core.py" in filepath:
                status = "PASSED" # Assume tests in test_core.py pass if 'core' suite targeted
            elif "memory" == keyword_filter and "test_memory.py" in filepath:
                status = "PASSED"
    ...
    def _sim_generate_pytest_output(self, results: List[Tuple[str,str]], verbose: bool, duration: float) -> Tuple[str, str, int]:
        """ Generates simulated pytest stdout, stderr, and exit code. """
        stdout_lines = ["============================= test session starts =============================="]
        stdout_lines.append(f"platform sim -- Python sim, pytest sim, pluggy sim")
        stdout_lines.append(f"rootdir: {self._get_current_cwd()}")
        stdout_lines.append("collected {} items".format(len(results)))
        stdout_lines.append("")

        passed_count = 0
        failed_count = 0
        
        for status, testname_full in results:
            short_status_char = "." if status == "PASSED" else "F"
            if verbose:
                stdout_lines.append(f"{testname_full} {status}")
            else:
                # In non-verbose, pytest just prints chars, then full names for failures
                # This is a simplification
                stdout_lines.append(f"{testname_full} {short_status_char}")
        ...
        if not results:
            stdout_lines.append("============================== no tests ran ==============================")
            return "\n".join(stdout_lines), "", 0 # Or exit code 5 for no tests selected

        # Recalculate counts correctly
        passed_count = sum(1 for r in results if r[0] == "PASSED")
        ...
        if summary_parts:
            ...
            if failed_count > 0:
                 for status, testname_full in results:
                      if status == "FAILED":
                           stdout_lines.append(f"FAILED {testname_full} - assert False (simulated)") # Simplified failure reason

            stdout_lines.append("========================= {} in {:.2f}s =========================".format(", ".join(summary_parts), duration))
        else: # Should only happen if no tests were run but results list was not empty (edge case)
            stdout_lines.append("============================== no tests found ==============================")
        ...
        try:
            ...
            if cmd == 'pwd':
                ...
                if target_path and target_path in fs:
                    ...
                    if item_info.get('type') == 'directory': parent_path_str = target_path if target_path != '/' else target_path; contents = [PurePosixPath(n).name for n, f_entry in fs.items() if str(PurePosixPath(n).parent) == parent_path_str and n != '/']; result['stdout'], result['success'], result['exit_code'] = "\n".join(sorted(contents)), True, 0
                ...
                if target_p:
                 ...
                 if target_p and target_p in fs:
                     ...
                     else: del fs[target_p]; result['success'], result['exit_code'] = True, 0
                ...
                while idx < len(args):
                    if args[idx] == '>' or args[idx] == '>>':
                        ...
                        if idx + 1 < len(args): target_file_str = args[idx+1]; content_to_echo = " ".join(args[:idx]); idx = len(args)
                        else: result['stderr'],result['reason']="echo: missing target file for redirection",'missing_args'; return result
                        break
                    idx += 1
                if not redirect_mode: content_to_echo = " ".join(args)
                ...
                if not redirect_mode: result['stdout'], result['success'], result['exit_code'] = content_to_echo, True, 0
                else:
                    ...
                    if not resolved_target_path: result['stderr'], result['reason'] = f"echo: Invalid path resolution for redirection target: {target_file_str}", 'invalid_path'; return result
                    final_write_path = resolved_target_path; parent_dir = str(PurePosixPath(resolved_target_path).parent); filename = PurePosixPath(resolved_target_path).name
                    existing_match_path = find_match_func(parent_dir, filename)
                    if existing_match_path:
                        if existing_match_path in fs and fs[existing_match_path].get('type') == 'directory': result['stderr'], result['reason'] = f"echo: {target_file_str}: Is a directory", 'is_directory'; return result
                ...
                final_dest_path = resolved_dest_arg; dest_parent_for_match = str(PurePosixPath(resolved_dest_arg).parent); dest_filename_for_match = PurePosixPath(resolved_dest_arg).name
                ...
                if existing_dest_dir_match and existing_dest_dir_match in fs and fs[existing_dest_dir_match].get('type') == 'directory':
                    src_basename = PurePosixPath(actual_src_path).name; path_in_dir = str(PurePosixPath(existing_dest_dir_match) / src_basename)
                ...
                elif existing_dest_dir_match and existing_dest_dir_match in fs : final_dest_path = existing_dest_dir_match
                ...
                src_info_copy = copy.deepcopy(fs[actual_src_path])
                resolved_dest_arg = resolve_path_func(dest_arg)
                ...
                if final_dest_path in fs:
                    ...
                    if src_info_copy.get('type') == 'directory':
                        ...
                        elif dest_info_at_final.get('type') == 'directory':
                            is_dest_empty = not any(str(PurePosixPath(k).parent) == final_dest_path for k in fs if k != final_dest_path and k != actual_src_path)
                            if not is_dest_empty: result['stderr'], result['reason'] = f"mv: failed to move '{src_arg}' to '{dest_arg}': Directory not empty ('{final_dest_path}')", 'directory_not_empty'; return result
                ...
                fs[final_dest_path] = data_to_move; fs[final_dest_path]['mtime'] = time.time()
                result['success'], result['exit_code'] = True, 0
            # --- START: pytest simulation ---
            elif cmd == 'pytest':
                pytest_start_time = time.monotonic()
                keyword_filter = None
                verbose_mode = False
                target_paths_args = [] # Files or directories specified in pytest command
                ...
                while idx < len(args):
                    if args[idx] == '-k':
                        ...
                        target_paths_args.append(args[idx])
                    idx += 1
                
                logger.debug(f"Simulating pytest: Targets={target_paths_args}, Keyword='{keyword_filter}', Verbose={verbose_mode}")

                discovered_sim_tests = self._sim_discover_tests(fs, target_paths_args, keyword_filter)
                
                sim_test_results = []
                if not discovered_sim_tests and keyword_filter: # If -k was used and nothing found
                    result['stdout'] = f"============================= test session starts ==============================\nplatform sim -- Python sim, pytest sim, pluggy sim\nrootdir: {current_cwd}\ncollected 0 items / 0 deselected\n\n============================ no tests ran due to deselection ============================\n"
                    result['exit_code'] = 5 # pytest exit code for no tests collected
                    result['success'] = True # Command itself ran, but no tests executed
                    result['reason'] = 'no_tests_collected'
                elif not discovered_sim_tests:
                     result['stdout'] = f"============================= test session starts ==============================\nplatform sim -- Python sim, pytest sim, pluggy sim\nrootdir: {current_cwd}\ncollected 0 items\n\n================================= no tests found ==================================\nERROR: InvocationError for command {command_str} (pytest exited with code 5.)" # More accurate
                     result['exit_code'] = 5
                     result['success'] = True
                     result['reason'] = 'no_tests_found'

                else:
                    for test_file_path, test_name in discovered_sim_tests:
                        status, full_name = self._sim_run_one_test(test_file_path, test_name, keyword_filter)
                        sim_test_results.append((status, full_name))
                    
                    pytest_duration = time.monotonic() - pytest_start_time
                    stdout_res, stderr_res, exit_code_res = self._sim_generate_pytest_output(sim_test_results, verbose_mode, pytest_duration)
                    
                    result['stdout'] = stdout_res
                    result['stderr'] = stderr_res
                    result['exit_code'] = exit_code_res
                    result['success'] = (exit_code_res == 0 or exit_code_res == 5) # Exit code 5 is "success" in the sense that pytest ran
                    result['reason'] = 'tests_ran' if exit_code_res in [0,1] else 'no_tests_collected' if exit_code_res == 5 else 'execution_error'
            # --- END: pytest simulation ---
            else:
    ...
    def _execute_real_command(self, command_str: str, execution_context: Callable[[str], Dict]) -> Dict[str, Any]:
        ...
        try:
             if command == 'cd':
                 ...
                 if target_dir_str == '~': target_dir = '/app'
    ...
    def read_file(self, path: str) -> Dict[str, Any]:
        ...
        if not attempt1_result['success'] and attempt1_result['reason'] == 'file_not_found':
            ...
            try:
                ...
                else: result.update(attempt1_result)
